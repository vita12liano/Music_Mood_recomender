...
Version adapted to use Vitaliano's recommender_05.recommend_playlist
(with signature:
    recommend_playlist(mood, activity, part_of_day, weather,
                       age, explorer, n=10,
                       fav_artists=None, language_prefs=None)
)
"""
...
# Import recommender (USA LA TUA VERSIONE)
if BASE_DIR not in sys.path:
    sys.path.append(BASE_DIR)

from recommender_05 import recommend_playlist, df  # <-- tua funzione / dataset

# Valid values (compatibili con recommender_05.build_target_profile)
VALID_MOODS = [
...
VALID_TIMES = ['morning', 'afternoon', 'night', 'late night', 'evening']
VALID_WEATHER = ['sunny', 'clear', 'rainy', 'storm', 'stormy', 'snow', 'snowy', 'cloudy']
...
# =======================
# ADVANCED PREFS (LANG, RECENCY, DURATION, DANCE)
# NB: i filtri avanzati vengono raccolti ma NON passati a recommender_05
# (che non li accetta in firma). Si possono usare in futuro per post-filter.
# =======================
...
async def handle_language_button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Qui in origine si passava a RECENCY_PREF; lo manteniamo
    keyboard = [
...
async def generate_playlist_final(message, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    QUI chiamiamo la TUA recommend_playlist di recommender_05,
    con la sua firma originale (senza recency/duration/danceability).
    """
    ...
    # advanced prefs raccolte ma NON usate dal recommender
